...
IMPORTANT: A partir da versão de 30 de setembro de 2024, essa limitação foi removida.
...
==== A verificação de ransomware dos seus backups na nuvem não é compatível

A verificação de backups na nuvem para ransomware não é suportada ao usar o mecanismo Podman. Se você estiver usando o recurso DataLock & ransomware para seus backups na nuvem, será necessário desativar as varreduras de ransomware. link:task-manage-backup-settings-ontap.html#enable-or-disable-ransomware-scans["Veja como desativar a verificação de ransomware"].
