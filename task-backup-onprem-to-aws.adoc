...
.Passos
. Confirme se as seguintes permissões do S3 (a partir da última https://docs.netapp.com/us-en/bluexp-setup-admin/reference-permissions-aws.html["Política de BlueXP "^]) fazem parte da função do IAM que fornece permissões ao conetor. Se não estiverem, consulte https://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies_manage-edit.html["Documentação da AWS: Editando políticas do IAM"].
+
[%collapsible]
====
[source, json]
